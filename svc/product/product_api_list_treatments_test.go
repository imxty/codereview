...
type ListTreatmentsTestSuite struct {
	suite.Suite
...
func (suite *ListTreatmentsTestSuite) SetupSuite() {
...
// TestGetAllIsTrue
func (suite *ListTreatmentsTestSuite) TestListTreatments() {
	...
	req := &productpb.ListTreatmentsRequest{
		OrganizationId: organizationId1,
		IsPublish:      true,
	}
	resp := &productpb.ListTreatmentsResponse{}
	err := suite.hdl.ListTreatments(ctx, req, resp)
	suite.Assert().NoError(err)
}

// TestOrganizationIdIsNull
func (suite *ListTreatmentsTestSuite) TestOrganizationIdIsNull() {
	t := suite.T()
	ctx := context.Background()
	req := &productpb.ListTreatmentsRequest{
...
// TestGetAllIsFalse
func (suite *ListTreatmentsTestSuite) TestGetAllIsFalse() {
	ctx := context.Background()
	suite.review.On("ListReviewResultByTargetIds", mock.Anything, mock.Anything).Return(
		&reviewpb.ListReviewResultByTargetIdsResponse{
			Results: make(map[string]*reviewpb.ReviewResult)}, nil).After(utils.RpcLatency())
...
func (suite *ListTreatmentsTestSuite) TearDownSuite() {}

func TestListTreatmentsTestSuite(t *testing.T) {
	suite.Run(t, new(ListTreatmentsTestSuite))
}
