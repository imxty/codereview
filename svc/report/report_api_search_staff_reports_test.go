...
type SearchStaffReportsTestSuite struct {
	suite.Suite
	hdl  *ReportAPIHandler
	user *usermock.UserAPIService
}

func (suite *SearchStaffReportsTestSuite) SetupSuite() {
...
// TestSearchStaffReports
func (suite *SearchStaffReportsTestSuite) TestSearchStaffReports() {
	...
	suite.user.On("BatchGetStaffs", mock.Anything, mock.Anything, mock.Anything).
		Return(&userpb.BatchGetStaffsResponse{
			Staffs: make(map[string]*userpb.Staff),
		}, nil).After(utils.RpcLatency())
	req := &reportpb.SearchStaffReportsRequest{
	...
	resp := &reportpb.SearchStaffReportsResponse{}
	err := suite.hdl.SearchStaffReports(ctx, req, resp)
	suite.Assert().NoError(err)
	suite.Assert().NotNil(resp)
}

func (suite *SearchStaffReportsTestSuite) TearDownSuite() {
}

func TestSearchStaffReportsTestSuite(t *testing.T) {
	suite.Run(t, new(SearchStaffReportsTestSuite))
}
