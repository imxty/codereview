...
import (
	...
	organizationadmin "github.com/jinmukeji/huimaibao-service/api/organization"
...
func main() {
	...
	// 加入jwt的检测
	wrapper := organizationadmin.NewAuthWrapper(&tokenStore, nil)
...
func appRegister(service micro.Service) service.RegisterServerFunc {
	return func(srv server.Server) error {
		...
		adminAPI := organizationadmin.NewOrganizationAPIHandler(userAPI, customerAPI, reviewAPI, reportAPI, notificationAPI, productAPI, tokenStore, s3Domain, redisCli)

		if err := pb.RegisterProductAPIHandler(srv, adminAPI); err != nil {
			return err
		}
		...
		if err := pb.RegisterUserAPIHandler(srv, adminAPI); err != nil {
			return err
		}
